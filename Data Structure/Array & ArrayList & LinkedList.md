...
- 삽입, 삭제 연산
  ...
  - 기존 배열의 중간에 새로운 값을 추가하는 연산 불가 ex. `array[2] = 5;`는 새로운 값을 대입하여 기존 값을 덮어씀
  - 별도의 삭제 연산이 없어 `null` 값 또는 유효하지 않은 값으로 대체 ex. `array[2] = null;` `array[2] = -987654321`
...
➡️ 삽입/삭제 연산만이 발생하는 스택, 큐 구현 시 `ArrayList` 보다 `LinkedList`를 사용하는 것이 좋음
