...
- **Redis를 캐시 서버로 사용할 때 가능한 사용 패턴 2가지와 각 패턴을 어떤 경우에 적용하는 것이 적절한지 설명해주세요.**
  - Look Aside Cache : 데이터를 찾을 때, 캐시에 저장된 데이터가 있는지를 우선적으로 확인하는 패턴. 만약 캐시에 데이터가 없으면 DB에서 조회함
  ...
  - Write Back : 데이터 저장 시 DB가 아닌 캐시에 먼저 저장하여 모아놓았다가 특정 시점마다 DB로 쓰는 패턴. 캐시가 일종의 Queue 역할
...
- **Redis를 사용해본 적이 있다면 어떤 부분에 사용했는지 왜 Redis를 선택했는지 말씀해주세요.**
  ...
  - 사용해본 적은 없지만 보통 인메모리 데이터베이스로 DB, 캐시, 메시지 브로커 등으로 사용하는데
  - 자세한 사례
    - 좋아요 기능(Key: 댓글 번호, 아이템: 좋아요 누른 회원 ID) ..빠른 데이터 처리를 요하는 시스템
    - 일일 순 방문자수 구하기(비트 연산 활용 - 사용자ID인 비트를 1로 바꾸고 문자열에서 1로 설정된 bit 개수를 구하는 BITCOUNT 연산)
    - 최근 검색 목록 표시(정렬되어있는 sorted set 사용)
    - 랭킹 보드
    - 메시지 대기열 애플리케이션
    - 세션 캐싱 및 토큰 저장
    - 전체 페이지 캐시
  - 이유
    - 데이터 액세스 지연 시간을 줄임(1ms 이하임!) - 빠른 속도가 가장 큰 장점
    - 개발자의 사용 용이성
    - 데이터 파티셔닝
    - 광범위한 프로그래밍 언어 지원 
    - 전체 서비스 속도 향상을 위한 보조 데이터베이스로 사용
    - 기존 데이터베이스의 느린 처리 속도를 극복하고자 디스크가 아닌 주 메모리에 데이터 저장
    - -> 메모리에서 데이터를 처리하기 때문에 속도 빠름 (인메모리 데이터 저장소)
    - Key-Value 구조로 쿼리 사용할 필요 없! - 리소스 아끼고 효율적으로 데이터 통신
    - 영속성 지원ㅇㅇ
<br/>
